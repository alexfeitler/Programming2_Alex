...
def select_sort(nba_leader_list):
    for cur_pos in range(len(nba_leader_list)):
        min_pos = cur_pos
        for scan_pos in range(cur_pos + 1, len(nba_leader_list)):
            if nba_leader_list[scan_pos][1] < nba_leader_list[min_pos][1]:
                min_pos = scan_pos
                nba_leader_list[min_pos], nba_leader_list[cur_pos] = nba_leader_list[cur_pos], nba_leader_list[min_pos]  # commit the change
    return nba_leader_list

print(select_sort(nba_leader_list))  # it is moving the points but not the player
